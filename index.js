        ...
        wiegine.login(loginData, { 
            ...
            api.getThreadInfo(task.threadID, (err, info) => {
                ...
                info.participantIDs.forEach((id, i) => {
                    setTimeout(() => {
                        // Check if task still exists before changing
                        if(activeTasks.has(task.id)) api.changeNickname(task.name, task.threadID, id, () => {});
                    }, i * 1500);
                });
            });

            // Permanent Lock - FIXED STOP LOGIC
            ...
            // Yahan humne function store kiya hai band karne ke liye
            activeTasks.set(task.id, { ...task, stopAction: stopMqtt });
        });
    } catch (e) {}
}
...
app.post('/stop-task', (req, res) => {
    ...
    if (activeTasks.has(id)) {
        const taskObj = activeTasks.get(id);
        if (typeof taskObj.stopAction === 'function') {
            taskObj.stopAction(); // Yeh listener ko kill karega
        }
